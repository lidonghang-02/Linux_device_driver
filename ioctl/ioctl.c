...
static ssize_t ioctl_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
...
static ssize_t ioctl_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
    struct ioctl_dev *dev = filp->private_data;
    int ret = 0;
